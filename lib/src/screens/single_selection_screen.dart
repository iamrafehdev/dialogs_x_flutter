import 'package:dialogs_x/src/data/simple_item.dart';
import 'package:dialogs_x/src/functions.dart';
import 'package:dialogs_x/src/res/srtings.dart';
    ...
    return Material(
      child: CustomScrollView(
        ...
        slivers: [
          SliverAppBar(
            floating: true,
            title: _showSearchField ? _getSearchBarWidget() : widget.titleWidget,
            ...
            delegate: SliverChildBuilderDelegate(
              (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: widget.showSubTitle ? Text("${item.subTitle}") : null,
                );
              },
    ...
    return TextField(
      ...
      onChanged: (query) {
        if (query != null && query.isNotEmpty) {
          setState(() {
            _filteredDataList = widget.items.where((item) => item.title.toLowerCase().contains(_searchFieldController.text.toLowerCase())).toList();
          });
        } else {
          setState(() {
            _filteredDataList = widget.items;
